...
export function ConvertTimeStringToObject(value: string, ownProperty?: string) {
  ...
  const lastDateValue = objectPath.get(lastDate, lastConfig.Value);

  if (!(lastDateValue instanceof Date)) {
    return value;
  }

  const timeSplit = split(value, ":");
  ...
  if (timeSplit.length === 2 || timeSplit.length === 3) {
    ...
    const min = toNumber(timeSplit[1]);
    const sec = toNumber(timeSplit.length === 3 ? timeSplit[2] : 0);
    if (
      isFinite(hour) &&
      isFinite(min) &&
      isFinite(sec) &&
    ...
    if (!ownProperty) {
      const lastDateOriginal = objectPath.get(lastDate, lastConfig.Original);
      objectPath.set(
        lastDate,
        lastConfig.Original,
        `${lastDateOriginal} ${value}`
      );
    }
  }
...
export function ConvertDateStringToObject(
  ...
  const internValue = trim(toLower(value));
  const parsedObject = {};
  lastConfig = config;
  ...
  if (startsWith(internValue, "from")) {
    objectPath.set(
      parsedObject,
      config.From,
      ParseDate(trimStart(internValue, "from "), config)
    );
    lastDate = parsedObject;
    return parsedObject;
  }
  ...
  if (startsWith(internValue, "to")) {
    objectPath.set(
      parsedObject,
      config.To,
      ParseDate(trimStart(internValue, "to "), config)
    );
    lastDate = parsedObject;
    return parsedObject;
  }
...
function ParseDate(
  date: string,
  config: ConvertToDate,
  markAsBetweenIfNotExact = false
) {
  let calendarType = "";
  const result = {};
  const gregorian = date.match(/(@#dgregorian@)(.*)/);
  const julian = date.match(/(@#djulian@)(.*)/);
  const hebrew = date.match(/(@#dhebrew@)(.*)/);
  ...
  const markerRegex = /(abt |cal |est |aft |bef |int )(.*)/;
  ...
  if (!groups) {
    ...
    if (markAsBetweenIfNotExact) {
      const internResult = {} as any;
      ConvertStringToDate(date, internResult, config, calendarType);

      const hasYear = objectPath.get(internResult, config.HasYear);
      const hasMonth = objectPath.get(internResult, config.HasMonth);
      const hasDay = objectPath.get(internResult, config.HasDay);
      ...
      // calculate to value
      const and = {} as any;
      const startDate = objectPath.get(internResult, config.Value);
      if (!hasMonth) {
        // last day of year
        if (calendarType === "Hebrew") {
          ConvertStringToDate(
            `1 nsn ${toNumber(date) + 1}`,
            and,
            config,
            calendarType
          );
        ...
        if (calendarType === "Hebrew") {
          ...
          const andDate = dateValue.toDate();
  ...
  while (groups != null && groups.length === 3) {
    const marker: string = groups[1];
    const dateContent: string = groups[2];
...
function ConvertFromJulian(date: Dayjs) {
  const newDateString = split(
    CalendarConverter.fromJulianToGregorian(
      date.get("year"),
      date.get("month"),
      date.get("date")
    ),
