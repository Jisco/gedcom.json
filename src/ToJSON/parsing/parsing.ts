...
export function ParseText(
  ...
  forEach(lines, (line, index) => {
    const dots = ".".repeat(index + 1);
    const left = lines.length - index + 1;
    const empty = " ".repeat(left);

    /* need to use  `process.stdout.write` \r clear the current line and then print the other char making it looks like it refresh*/
    process.stdout.write(
      `\r[${dots}${empty}] ${((index + 1) * 100) / lines.length}%`
    );
