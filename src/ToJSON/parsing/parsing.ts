...
export function ParseText(
  ...
  const nextLine = (lastLine: ParsedLine | undefined = undefined) => {
    if (lastLine) {
...
export function ParseFile(
  ...
  const linesCountLr = new LineByLineReader(path);
  let linesCount = 0;
  linesCountLr.on("line", function () {
    linesCountLr.pause();
    linesCount++;
    linesCountLr.resume();
  });
...
export function ProcessNewLine(
  ...
  const actualLine = trimStart(line);
  ...
  const processingResult = ProcessLine(parsedLine, lastLevel);
  if (processingResult.Parsed) {
    ...
    if (parsedLine.Tag && parsedLine.Tag[0] === "_") {
      stats.NotParsedLinesWithoutGEDCOMTag.push(
        new StatisticLine(lineNumber, actualLine, processingResult.Reason)
      );
    } else {
      stats.NotParsedLines.push(
        new StatisticLine(lineNumber, actualLine, processingResult.Reason)
      );
    }
  }
