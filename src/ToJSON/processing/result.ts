...
export function CreateResult(objects: ParsingObject[]) {
  ...
  forEach(mainObjects, (mainObject) => {
    SetObject(
      result,
      mainObject.Informations.PropertyPath,
      mainObject.Result,
      mainObject.Informations.Definition
    );
  });
...
export function CreateMainObject(objects: ParsingObject[]): MainObject {
  ...
  // concat objects
  const mergeWithLastValue = filter(
    objects,
    (x) => x.Definition.MergeWithLast === "true"
  ) as ParsingObject[];
  let mergeValue = "";

  forEachRight(mergeWithLastValue, (obj) => {
    const index = indexOf(objects, obj);
    const objectsToSearch = take(objects, index);
    const mergeObject = last(objectsToSearch);
    ...
    if (mergeObject.StartLevel > 0 && isObject(mergeObject.Object)) {
      const key = last(keys(mergeObject.Object));

      if (!key) {
        return;
      }

      const value = objectPath.get(mergeObject.Object, key);
  ...
  if (!isEmpty(mergeValue)) {
    const index = indexOf(objects, first(mergeWithLastValue));
    const objectsToSearch = take(objects, index);
    const mergeObject = last(objectsToSearch);
  ...
  const mergeWithLast = filter(
    objects,
    (x) => x.Definition.MergeWithLast
  ) as ParsingObject[];
  ...
  forEachRight(mergeWithLast, (obj) => {
    ...
    const mergeObject = findLast(
      objectsToSearch,
      (x) => x.Definition.Tag === obj.Definition.MergeWithLast
    );
  ...
  const mergeWithNext = filter(
    objects,
    (x) => x.Definition.MergeWithNext
  ) as ParsingObject[];

  forEachRight(mergeWithNext, (obj) => {
    const index = indexOf(objects, obj);
    const objectsToSearch = slice(objects, index);
    const mergeObject = find(
      objectsToSearch,
      (x) => x.Definition.Tag === obj.Definition.MergeWithNext
    );
  ...
  forEach(objects, (o) => {
    const obj = o as ParsingObject;
    const partPath: string[] = [];
    ...
    forEach(dropRight(obj.PropertyPath), (property: any) => {
      ...
      const value = objectPath.get(result, partPath);

      // If path value is an array, add position in array to path
      if (isArray(value)) {
        obj.PropertyPath.splice(
          indexOf(obj.PropertyPath, property) + 1,
          0,
          `${value.length - 1}`
        );
...
export function GetMainObjectIndexes(objects: ParsingObject[]) {
  ...
  const mainObjects = filter(objects, (x) => x.StartLevel === 0);
  const mainObjectIndexes: number[] = [];

  forEach(mainObjects, (mainObject) => {
    mainObjectIndexes.push(indexOf(objects, mainObject));
  });

  const mainObjectIndexObject = [];

  for (let i = 0; i < mainObjects.length; i++) {
    const startIndex = mainObjectIndexes[i];
    let endIndex = 0;
...
export function SetObject(
  ...
  const oldValue = objectPath.get(mainObject, path);
...
export function SetOrCreateArray(
  obj: any,
  path: string[],
  definition: TagDefinition,
  value: any
  ...
  if (!objectPath.has(obj, path)) {
    ...
    const parentPath = dropRight(fclone(path));
    if (objectPath.has(obj, parentPath)) {
      const parentValue = objectPath.get(obj, parentPath);
  ...
  const oldValue = objectPath.get(obj, path);

  if (definition.ConvertTo instanceof ConvertToString) {
    const newLineCharacter = definition.ConvertTo.NewLineCharacter;
