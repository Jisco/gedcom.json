...
export function ManipulateValue(definition: TagDefinition, line: ParsedLine) {
  let value: string = trim(
    isEmpty(line.ReferenceId) ? line.Value : line.ReferenceId
  );
  const convertTo = definition.PropertyType ?? definition.ConvertTo;
  ...
  if (definition.Replace) {
    const pattern = definition.Replace.Value;
    const replacement = definition.Replace.With;
