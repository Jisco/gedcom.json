...
export function Convert(argv) {
  ...
  parse
    ...
    .then((result) => {
      if (argv.out) {
        parse.SaveAs(result.Object, argv.out);
      } else {
        if (silent) {
          return;
        }
      ...
      console.log(
        `Not Parsed Lines because of user defined Tag: ${result.Statistics.NotParsedLinesWithoutGEDCOMTagCount}`
      );
      ...
      if (result.Statistics.IncorrectLinesCount > 0) {
        ...
        console.log('-----------------------------------------------');
        result.Statistics.IncorrectLines.forEach((line) => {
