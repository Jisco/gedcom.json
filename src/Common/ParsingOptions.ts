export default class ParsingOptions {
  ...
  GetConfig() {
    return (
      this.config ??
      require("fs").readFileSync("options/version551.yaml", "utf8")
    );
  }

  SetProgressFunction(
    func: (propertiesCount: number, actualProperty: number) => void
  ) {
    this.progressFunction = func;
  }

  GetProgressFunction():
    | ((propertiesCount: number, actualProperty: number) => void)
    | undefined {
